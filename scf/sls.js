  ...
  await k2c(
    httpProxy({
      ...
      onProxyReq: (proxyReq, req, res) => {
        ...
        proxyReq.setHeader(
          "User-Agent",
          "Android  com.kuangxiangciweimao.novel  fake_server_by_scf"
        );
      },
      onProxyRes: (proxyRes, req, res) => {
        ...
        Object.assign(res, {
          ...
          write: (chunk) => {
            ...
            const headers = Object.keys(proxyRes.headers).reduce(
              (prev, curr) => {
                let value = "";
...
router.get(`/html/*`, async (ctx) => {
  await sendFile(ctx, path.join(__dirname, "indexidentity.html"));
});
